...
<form action="{{ route('admin.servers.view.startup', $server->id) }}" method="POST">
        ...
        <div class="col-xs-12">
            <div class="box box-primary">
                ...
                <div class="box-body">
                    ...
                    <p class="small text-muted">在此編輯您的伺服器啟動指令。預設情況下可使用以下變數：<code>@{{SERVER_MEMORY}}</code>、<code>@{{SERVER_IP}}</code> 和 <code>@{{SERVER_PORT}}</code>。</p>
                </div>
                <div class="box-body">
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">服務配置</h3>
                </div>
                <div class="box-body row">
                    <div class="col-xs-12">
                        <p class="small text-danger">
                            更改以下任何值將導致伺服器處理重新安裝指令。伺服器將被停止，然後繼續進行。
                            如果您不希望服務指令碼執行，請確保勾選底部的方塊。
                        </p>
                        <p class="small text-danger">
                            <strong>在許多情況下，這是一個破壞性操作。此伺服器將立即停止，以便進行此操作。</strong>
                        </p>
                    </div>
                    ...
                    <div class="form-group col-xs-12">
                        ...
                        <p class="small text-muted no-margin">選擇將為此伺服器提供處理資料的 Egg。</p>
                    </div>
                    <div class="form-group col-xs-12">
            ...
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">Docker 鏡像配置</h3>
                </div>
                <div class="box-body">
                    <div class="form-group">
                        <label for="pDockerImage">鏡像</label>
                        <select id="pDockerImage" name="docker_image" class="form-control"></select>
                        <input id="pDockerImageCustom" name="custom_docker_image" value="{{ old('custom_docker_image') }}" class="form-control" placeholder="或輸入自定義鏡像..." style="margin-top:1rem"/>
                        <p class="small text-muted no-margin">這是用於執行此伺服器的 Docker 鏡像。從下拉選單中選擇一個鏡像，或在上面的文字欄位中輸入自定義鏡像。</p>
                    </div>
                </div>
            </div>
        </div>
...
@section('footer-scripts')
    ...
    $(document).ready(function () {
        $('#pEggId').select2({placeholder: '選擇 Nest Egg'}).on('change', function () {
            ...
            if (!_.get(objectChain, 'startup', false)) {
                $('#pDefaultStartupCommand').val(_.get(parentChain, 'startup', '錯誤：未定義啟動！'));
            } else {
        ...
        $('#pNestId').select2({placeholder: '選擇 Nest'}).on('change', function () {
            $('#pEggId').html('').select2({
