    ...
    <div class="col-xs-12">
        <div class="nav-tabs-custom nav-tabs-floating">
            <ul class="nav nav-tabs">
                ...
                <li><a href="{{ route('admin.nodes.view.allocation', $node->id) }}">配置</a></li>
            ...
            <div class="box">
                ...
                <div class="box-body row">
                    ...
                    <div class="form-group col-xs-12">
                        ...
                        <p class="text-muted"><small>請輸入網域名稱（例如 <code>node.example.com</code>）以用於連接到守護程式。只有在您不對此節點使用 SSL 時，才能使用 IP 位址。
                                <a tabindex="0" data-toggle="popover" data-trigger="focus" title="為什麼我需要 FQDN？" data-content="為了保護您的伺服器與此節點之間的通信，我們使用 SSL。我們無法為 IP 位址生成 SSL 證書，因此您需要提供 FQDN。">為什麼？</a>
                            </small></p>
                    </div>
                    <div class="form-group col-xs-12">
                        <label class="form-label"><span class="label label-warning"><i class="fa fa-power-off"></i></span> 透過 SSL 通信</label>
                        <div>
                            <div class="radio radio-success radio-inline">
                                <input type="radio" id="pSSLTrue" value="https" name="scheme" {{ (old('scheme', $node->scheme) === 'https') ? 'checked' : '' }}>
                                <label for="pSSLTrue"> 使用 SSL 連接</label>
                            </div>
                            <div class="radio radio-danger radio-inline">
                                <input type="radio" id="pSSLFalse" value="http" name="scheme" {{ (old('scheme', $node->scheme) !== 'https') ? 'checked' : '' }}>
                                <label for="pSSLFalse"> 使用 HTTP 連接</label>
                            </div>
                        </div>
                        <p class="text-muted small">在大多數情況下，您應該選擇使用 SSL 連接。如果使用 IP 位址或您不希望使用 SSL，請選擇 HTTP 連接。</p>
                    </div>
                    <div class="form-group col-xs-12">
                        <label class="form-label"><span class="label label-warning"><i class="fa fa-power-off"></i></span> 位於代理伺服器後面</label>
                        <div>
                            <div class="radio radio-success radio-inline">
                                <input type="radio" id="pProxyFalse" value="0" name="behind_proxy" {{ (old('behind_proxy', $node->behind_proxy) == false) ? 'checked' : '' }}>
                                <label for="pProxyFalse"> 不在代理伺服器後面 </label>
                            </div>
                            <div class="radio radio-info radio-inline">
                                <input type="radio" id="pProxyTrue" value="1" name="behind_proxy" {{ (old('behind_proxy', $node->behind_proxy) == true) ? 'checked' : '' }}>
                                <label for="pProxyTrue"> 在代理伺服器後面 </label>
                            </div>
                        </div>
                        <p class="text-muted small">如果您在代理伺服器（如 Cloudflare）後面執行守護程式，請選擇此選項，以使守護程式在啟動時跳過證書檢查。</p>
                    </div>
                    <div class="form-group col-xs-12">
                        ...
                        <p class="text-muted small">如果節點被標記為「維護中」，用戶將無法存取此節點上的伺服器。</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="box-body row">
                    <div class="col-xs-12">
                        <div class="row">
                            ...
                            <div class="form-group col-xs-6">
                                <label for="memory_overallocate" class="control-label">超配</label>
                                <div class="input-group">
                        ...
                        <p class="text-muted small">輸入此節點可用於分配給伺服器的總記憶體量。您也可以提供一個百分比，允許分配超過定義的記憶體。</p>
                    </div>
                    <div class="col-xs-12">
                        <div class="row">
                            <div class="form-group col-xs-6">
                            ...
                            <div class="form-group col-xs-6">
                                <label for="disk_overallocate" class="control-label">超配</label>
                                <div class="input-group">
                        ...
                        <p class="text-muted small">輸入此節點可用於伺服器分配的總磁碟空間。您也可以提供一個百分比，以決定超過設定限制的磁碟空間量。</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="box">
                ...
                <div class="box-body row">
                    <div class="form-group col-xs-12">
                        ...
                        <p class="text-muted"><small>輸入可透過網頁檔案管理器上傳的檔案最大大小。</small></p>
                    </div>
                    <div class="col-xs-12">
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="daemonListen" class="control-label"><span class="label label-warning"><i class="fa fa-power-off"></i></span> 守護程式連接埠</label>
                                <div>
                                    <input type="text" name="daemonListen" class="form-control" value="{{ old('daemonListen', $node->daemonListen) }}"/>
                                </div>
                            </div>
                            <div class="form-group col-md-6">
                                <label for="daemonSFTP" class="control-label"><span class="label label-warning"><i class="fa fa-power-off"></i></span> 守護程式 SFTP 連接埠</label>
                                <div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <p class="text-muted"><small>守護程式執行自己的 SFTP 管理容器，不使用主物理伺服器上的 SSHd 程式。<Strong>請勿使用與您物理伺服器 SSH 程式相同的連接埠。</strong></small></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="box box-primary">
                ...
                <div class="box-body row">
                    <div class="form-group col-sm-6">
                        <div>
                            <input type="checkbox" name="reset_secret" id="reset_secret" /> <label for="reset_secret" class="control-label">重設守護程式主密鑰</label>
                        </div>
                        <p class="text-muted"><small>重設守護程式主密鑰將使來自舊密鑰的任何請求無效。此密鑰用於守護程式上的所有敏感操作，包括伺服器建立和刪除。我們建議定期更換此密鑰以提高安全性。</small></p>
                    </div>
                </div>
